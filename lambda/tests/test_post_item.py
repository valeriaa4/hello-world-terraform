...
def test_post_item_success(mock_dynamodb_table, user_event, registry_body):
    ...
    assert body["item"]["PK"] == f"USER#{user_event['requestContext']['authorizer']['jwt']['claims']['sub']}"
...
def test_create_item_missing_fields(user_event):
    ...
    assert response["statusCode"] == 400
    assert "Campos 'name' e 'date' são obrigatórios." in json.loads(response["body"])["message"]
